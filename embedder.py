...
from dictionary import Dictionary, BOS_WORD, EOS_WORD, PAD_WORD, UNK_WORD, MASK_WORD
from utils import AttrDict


# import Trained_Model # -> import the model you want
# The proposed model will have the following attributes : 
# 1. accept the pretrained parameters and the dictionary (vocab)
# 2. has embeddings, positional embeddings, language embeddings
# 3. and will contain transformer and Self Attention layers 
# 4. the final embeddings can be extracted by passing the input through the model (forward pass), and the output from last layer contains the embeddings
